...
const makeRequest = async (filePath) => {
  ...
  let importTask = job.tasks.filter(task => task.name == 'import')[0]
  ...
  let convertedFileName = filePath.replace(/^.*[\\\/]/, '').replace(/\.tex$/, '.txt')

  request(url, {encoding: 'binary'}, function(error, response, body) {
    fs.writeFile(convertedFileName, body, 'binary', function (err) { console.log("Deu merda"); });
  });
